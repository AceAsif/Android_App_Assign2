...
import au.edu.utas.iqbalma.kit305_assign2.databinding.ActivityDotGameBinding
...
val items = mutableListOf<GameData>()
var databaseID = "" //This is for the id for data being stored in database

@RequiresApi(Build.VERSION_CODES.O)
class DotGame : AppCompatActivity() {
    private lateinit var ui : ActivityDotGameBinding
    ...
    var numRep = 0 //This is the total number of repetitions set by user and for for storing the shared preference value and using it in other places
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ui = ActivityDotGameBinding.inflate(layoutInflater)
        ...
        ui.lblEnteredText.text = userName //This is used to display the name in the textView
        ...
        /*These are getting the number of dots from the setting page using shared preference*/
        preferences = getSharedPreferences(Time_Preference, MODE_PRIVATE)
        val timeLimit = preferences.getInt("TIME_LIMIT", 60)
        ...
        ui.repView.text = "Repetition: $repCount out of $repNum"
        ...
        /*Time for the clock*/
        timeRemain = (timeLimit * 1000).toLong() //This is for creating the time in the timer
        startClock() //This is for starting the clock of the game.
        ...
        ui.button.setOnClickListener {view->
            ...
            var clickTime1 = dateTime1.format(DateTimeFormatter.ofPattern("d/M/y H:m:ss"))*/
            //var clickTime1 = SimpleDateFormat("HH:mm:ss").format(Date())
            ...
            //listButtons.add("$clickTime1 Button1")
        ...
        ui.button2.setOnClickListener {view->
            ...
            //var clickTime2 = SimpleDateFormat("HH:mm:ss").format(Date())
            var clickTime2 = SimpleDateFormat("ss.SSS").format(Date())
            //listButtons.add("$clickTime2 Button2")
        ...
        ui.quitBut.setOnClickListener {
            val builder = AlertDialog.Builder(ui.quitBut.context)
            with(builder){
            ...
            builder.setNegativeButton("No",
                DialogInterface.OnClickListener { _, id ->
                    //start the clock if the user presses "No"
                    startClock()
    ...
    fun gameCompletePage(){
        storeToDatabase()
        val complete = Intent(this, DotGameComplete::class.java)
    ...
    fun gameComplete() {
        Log.d("TAG_Check2","repCount = $repCount and numRep = ${numRep + 1}")
        if (repCount == (numRep + 1)){
            gameOver = "completed"
            //These parts are for getting the duration of the game
            val dateTime2  = LocalDateTime.now()
            endTime = dateTime2.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.LONG, FormatStyle.MEDIUM))
    ...
    fun resetGame(){
        with(ui){
            ...
            indicateCount = 0
            ui.progressBar.progress = 0
            repView.text = "Repetition: $repCount out of $numRep"
    ...
    private fun startClock(){
        clockTimer = object : CountDownTimer(timeRemain, 1000) {
            ...
            override fun onFinish() {
                ui.timeView.text = "Time: 0 : 00 min(s) Done!!"
                gameOver = "Incomplete"
                //These parts are for getting the duration of the game
                val dateTime2  = LocalDateTime.now()
                endTime = dateTime2.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.LONG, FormatStyle.MEDIUM))
    ...
    private fun updateClockInfo() {
        ...
        if(secs < 10 ){
            ui.timeView.text = "Time: $mins : 0$secs min(s)"
        }
        else {
            ui.timeView.text = "Time: $mins : $secs min(s)"
        }
    }
    ...
    fun storeToDatabase(){
        ...
        val attempt = GameData(
            ...
            repetitionNum = "${repCount -1} /$numRep",
            gameStatus = gameOver,
